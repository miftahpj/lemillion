    ...
    <section class="py-32 bg-[#0C0A07] relative">
      ...
      <div class="max-w-7xl mx-auto px-8">
        ...
        <div v-else class="text-center py-16 border" style="border-color:rgba(201,150,58,0.08);">
          <p class="font-serif text-xl italic" style="color:rgba(245,240,232,0.2);">Portfolio belum ada. Tambah data lewat neon/02_seed.sql atau panel admin.</p>
        </div>
    ...
    <section class="py-32 bg-[#0C0A07] relative">
      <div class="max-w-7xl mx-auto px-8">
        ...
        <div v-if="loading" class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div v-for="i in 3" :key="i" class="bg-[#0C0A07] border p-10 h-64 animate-pulse" style="border-color:rgba(201,150,58,0.08);" />
        </div>

        <div v-else-if="servicesList.length > 0" class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div
            v-for="(svc, idx) in servicesList" :key="svc.id"
            class="group relative bg-[#0C0A07] border p-10 transition-all duration-500 hover:-translate-y-1 hover:border-[#C9963A]/25 overflow-hidden"
